...
def FrankeFunction(x, y):
    term1 = 0.75 * np.exp(-((9*x - 2)**2)/4.0 - ((9*y - 2)**2)/4.0)+ np.random.normal(0, 0.05, x.shape)
    term2 = 0.75 * np.exp(-(9*x + 1)**2/49.0 - 0.1*(9*y + 1)) + np.random.normal(0, 0.05, x.shape)
    term3 = 0.5 * np.exp(-(9*x - 7)**2/4.0 - ((9*y - 3)**2)/4.0) + np.random.normal(0, 0.05, x.shape)
    term4 = -0.2 * np.exp(-(9*x - 4)**2 - (9*y - 7)**2) + np.random.normal(0, 0.05, x.shape)
...
# StandardScaler().fit(z_train)
# StandardScaler().fit(x_test)
# StandardScaler().fit(y_test)
# StandardScaler().fit(z_test)
...
for degree in range(Maxpolydegree):
    ...
    for i in range(n_boostraps):
        ...
        z_pred.ravel() #problemer her, må fikses. Får ikke beregnet error, bias og variance pga. dimension mismatch.
